...
usemathjax: true
thanks: "TODO"
asset_path: /assetsPosts/2024-06-12-zx-intro/
---
...
The copy spider produces copies of the Z basis states $|0\rangle$ and $|1\rangle$, while the XOR spider performs the XOR operation on the same basis, sending the 2-qubit Z basis state $|a\,b\rangle$ to the 1-qubit Z basis state $|a \oplus b\rangle$. Combined, the copy and XOR spiders form the CNOT gate, the most common 2-qubit gate used in digital quantum computing:
...
![Euler Rule]({{page.asset_path}}/rules-fusion.png)
...
The "copy" rule states that the spider states with angle $0$ and $\pi$ of one colour are copied by the copy spider of the other colour. The copy rule for Z spiders, copying the Z basis states $|0\rangle$ and $|1\rangle$ &mdash; the X spider states with phases $0$ and $\pi$, respectively &mdash; is shown below:
